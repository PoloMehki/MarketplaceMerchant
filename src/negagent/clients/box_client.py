...
class BoxArchiveClient:
    ...
    def __init__(
        ...
        if sdk is not None:
            self._sdk = sdk
        else:  # pragma: no cover — exercised only against real Box
    ...
    def archive_negotiation(self, state: NegotiationState) -> None:
        ...
        try:
            folder = self._sdk.folder(self._root_folder_id).create_subfolder(
                f"negotiation-{state.listing_id}"
            )
            ...
            pass  # Box failure must never break the negotiation
